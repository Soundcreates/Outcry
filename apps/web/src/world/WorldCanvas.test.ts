...
assert.match(source, /function leaveWorldRoom\(room: WorldRoom\)/);
assert.match(source, /void room\.leave\(room\.connection\.isOpen\)/);
assert.match(source, /leaveWorldRoom\(joinedRoom\)/);
assert.match(source, /if \(room\) leaveWorldRoom\(room\)/);
